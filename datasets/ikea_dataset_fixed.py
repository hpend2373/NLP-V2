...
class IKEADataset(Dataset):
        ...
        for furniture_id, furniture_data in self.metadata.items():
            ...
            for step_idx, step_data in enumerate(steps):
                sample = {
                    ...
                    # Manual-level data (no camera/pose here)
                    'manual_parts': step_data.get('manual_parts', []),
                    'manual_connections': step_data.get('manual_connections', []),
                    # Frame-level data (has camera/pose/masks)
    ...
    def _validate_sample(self, sample: Dict) -> bool:
        ...
        # Check if has parts to add (from manual-level)
        if not sample['manual_parts']:
            return False
    ...
    def _get_default_camera(self) -> Dict[str, np.ndarray]:
        """Get default camera parameters when no frame alignment is available"""
        # Default intrinsics
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        # Prepare components from manual-level data
        added_components = []
        part_meshes = []
        part_poses = []
        part_masks = []

        for part_info in sample['manual_parts'][:self.max_parts_per_step]:
            ...
            if part_id in frame_poses:
                ...
                # No frame-level pose available
                pose = {'R': np.eye(3), 't': np.zeros(3)}
