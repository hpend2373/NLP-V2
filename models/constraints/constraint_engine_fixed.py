"""
Constraint Engine for IKEA Assembly (FIXED VERSION)
Fixes all critical bugs in the original implementation
"""
...
class ConstraintEngine:
    ...
    def get_valid_poses(
        ...
        # FIX: Use enum value comparison instead of string
        if part_info.symmetry.symmetry_type != self.registry.SymmetryType.NONE:
    ...
    def _generate_connection_poses(
        ...
        for assembled_id, assembled_data in assembly.assembled_parts.items():
            ...
            for i, (conn_new, conn_existing) in enumerate(connection_pairs):
                # FIX: Use proper connection indexing
                if (assembled_id, i) in assembly.occupied_connections:
                    continue
        ...
        for assembled_id, assembled_data in assembly.assembled_parts.items():
            ...
            for surface_new in self._get_flat_surfaces(part_info.mesh):
                for surface_existing in self._get_flat_surfaces(assembled_mesh):
                    # FIX: surface_existing is already in world coordinates (transformed mesh)
                    # Don't transform normals again
    ...
    def _generate_symmetry_poses(
        ...
        try:
            ...
            # If not available, use string comparison
            SymmetryType = None

        for R, t, score in base_poses:
            # FIX: Compare with enum values properly
            if SymmetryType:
    ...
    def _align_surfaces(
        ...
        # FIX: Handle case where surface_existing is already in world space
    ...
    def _validate_pose(
        ...
        for assembled_id, assembled_data in assembly.assembled_parts.items():
            ...
            # FIX: Use proper bounding box overlap check
            # trimesh doesn't have bounds_overlap, use bounding box intersection
            ...
            # closest_point returns (closest_points, distances, triangle_ids)
    ...
    def _score_connection(
        ...
        # FIX: Transform connection points properly
        # conn_new needs to be transformed by (R, t)
        # conn_existing needs to be transformed by existing_pose
        ...
        # FIX: We want opposite normals, so closer to -1 is better
    ...
    def _score_surface_contact(
        ...
        # FIX: surface_existing normal might already be in world coordinates
